...
const createCaches = () => {
  if (
    Object.keys(malCache).length +
      Object.keys(mamCache).length +
      Object.keys(masCache).length >
    0
  ) {
    return;
  }
  ...
  if (Object.keys(malCache).length === 0) {
    console.time("MA-L");
    (oui as Array<MacVendor>).forEach(
      (vendor) => (malCache[vendor.Assignment] = vendor)
    );
  ...
  if (Object.keys(mamCache).length === 0) {
    console.time("MA-M");
    (mam as Array<MacVendor>).forEach(
      (vendor) => (mamCache[vendor.Assignment] = vendor)
    );
  ...
  if (Object.keys(masCache).length === 0) {
    console.time("MA-S");
    (oui36 as Array<MacVendor>).forEach(
      (vendor) => (masCache[vendor.Assignment] = vendor)
    );
